...
export function Header() {
    const { isLoggedIn, changeLoginStatus, username, role, likedfunds } = useContext(GlobalContext);
    const navigate = useNavigate();

    function logout() {

        fetch('http://localhost:5030/api/logout', {
            credentials: 'include',
        })
            .then(() => {
        ...
        <div className="container">
            <header className="d-flex flex-wrap align-items-center justify-content-center justify-content-md-between py-3 mb-4 border-bottom">
                ...
                <ul className="nav col-12 col-md-auto justify-content-center">
                    ...
                    <li>
                        <Link to='/funds' className="nav-link px-2">Istorijos</Link>
                    </li>
                    <li>
                ...
                {isLoggedIn && <div className="col-md-3 nav justify-content-end">
                    <p>{username} ({role})</p>
                    <Link to='/dashboard' className="nav-link px-2">Dashboard ({likedfunds.length})</Link>
